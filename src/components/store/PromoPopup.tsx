...
export default function PromoPopup() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      if (data) {
        ...
        // Only show if enabled and not seen this session
        const hasSeen = sessionStorage.getItem('velour_promo_v1')
        if (obj.promo_enabled === 'true' && !hasSeen) {
  ...
  const closePopup = () => {
    setIsOpen(false)
    sessionStorage.setItem('velour_promo_v1', 'true')
  }
